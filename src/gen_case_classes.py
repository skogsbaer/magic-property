...
CUSTOM_INT_TYPES = ['FRNetworkKind',
                    'AuthenticationResultStatus',
                    'LabelTextLineBreakMode',
                    'PushClientState',
                    'StructExprOp',
                    'StructExprAxis',
                    'StructPropertyType',
...
class Property(CommonEqualityMixin):
    ...
    def nsCodingTarget(self):
        ...
        if t in CUSTOM_INT_TYPES: return 'Integer'
    ...
    def fr_type(self):
        ...
        if t in CUSTOM_INT_TYPES: return 'FRInteger' + t
...
FRGenerateIsEqualHashDescription${props_len}($name,#foreach ($p in $props)
...
def gen_impl_def(iface, ext_opt, impl_opt):
    def f(opt, is_ext, lines, tmpl):
        ...
        if len(iface.props) > 0:
            code = tmpl.merge(tmpl_args(iface))
        else:
            code = ''
...
def gen_iface_def(iface):
    if len(iface.props) > 0:
        extra_content = h_template.merge(tmpl_args(iface))
    else:
        extra_content = ''
...
def gencode_for_file(h, m_opt, output_dir):
    h_lines = get_lines(h)
    ...
    if m_opt is not None:
        ...
        m_post_lines = []
    h_content = ''
    m_imports = [os.path.basename(h)] + M_FILE_IMPORTS
    m_content = ''.join(['#import "%s"\n' % x for x in m_imports])
    for iface in ifaces:
        iface.check()
        h_content += gen_iface_def(iface) + '\n'
        ...
        m_content += gen_impl_def(iface, ext_opt, impl_opt) + '\n'
    h_content += '\n'.join(h_post_lines)
...
def load_enums(enum_file):
    ...
    def enum(name, alts):
        ...
        i = 1
        for a in alts:
            alts_with_values.append(EnumAlt(name, a, i))
            i = i + 1
        enums.append(Enum(name, alts_with_values))
    d = os.path.dirname(enum_file)
    f = os.path.basename(enum_file)
    (m, _) = os.path.splitext(f)
    sys.path.insert(0, d)
    try:
        mod = __import__(m)
        mod.define_enums(enum)
    except ImportError, e:
        abort('Error loading enum definitions from file ' + enum_file + ': ' + str(e))
    return enums
...
@interface FRInteger${enum.name} : FRInteger
...
def gencode(input_files, output_dir, enum_file):
    if enum_file:
        ...
        enums = []
    global CUSTOM_INT_TYPES
    CUSTOM_INT_TYPES = CUSTOM_INT_TYPES + enums
    ...
    for h in hFiles:
        ...
        has_m = m in mFiles
        if has_m:
            info("Generating code for %s/%s" % (h, m))
        else:
            info("Generating code for %s" % h)
...
def main(argv):
    parser = argparse.ArgumentParser(prog='gen-case-classes',
                                     description='Generated code for case class definitions')
    ...
    parser.add_argument('--enums', metavar='FILE', required=False, nargs=1,
                        help='.py file with enum definitions')
    args = parser.parse_args(argv[1:])
    output_dir = args.output_dir[0]
    enum_file = None
    if args.enums:
        enum_file = args.enums[0]
    gencode(args.files, output_dir, enum_file)

if __name__ == '__main__':
    if len(sys.argv) >= 2 and sys.argv[1] == '--test':
        new_argv = [sys.argv[0] + ' --test'] + sys.argv[2:]
